...
//   alert(base_url);
  ...
  function validateField({
    ...
    const saveButton = $(".contact-update");
  ...
  $(".contact-update").on("click", function (e) {
    ...
    formData.append("remarks", $("#contact_remarks").val());
    formData.append("status", $("#contact_status").val());
    const saveUrl = base_url + "admin/Dashboard/update_contactus";
    // Submit AJAX request
    $.ajax({
      url: saveUrl,
      type: "POST",
        ...
        if (response.status) {
          ...
          setTimeout(() => {
            ...
            window.location.href =
              base_url + "admin/Dashboard/contactus";
          }, 2000);
        } else {
          alert("error");
        }
  ...
  $(document).on("change", "#contact_status_change", function () {
    const status = $(this).val();
    $.ajax({
        url: base_url + "admin/Dashboard/update_contact_status",
        type: "POST",
        data: { status: status },
        dataType: "json",
        success: function (response) {
            $("#contact_list_container").html(response.html);
            $("#contact_status_change").val(status);
